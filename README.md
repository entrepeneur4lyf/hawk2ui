...
| Runtime shader effects (`SkRuntimeEffect`) | Remediated | `hawk2ui-render-skia` now exposes a production runtime shader-effect API with bounded `SkSL` source compilation, stable effect IDs, typed float/int uniforms, registered image child shaders, compiled-effect cache stats, deterministic command evidence, capability reporting, and Skia pixel tests. |
