...
Split into the testable render-cycle logic (0009.4a) and the live windowing wiring (0009.4b). **The cadence is unproven until 0009.4b** — 4a builds and tests the per-frame cycle, but nothing drives it: the editor still presents the static construction-time scene until 4b wires the cycle into the Baseview frame loop. 4a-done is **not** 0009.4-done.
...
#### 0009.4b Live windowing wiring + cadence drivers — ⏳ NEXT (verification drops to a gated Xvfb smoke)

- [ ] Deliverable: thread a **scene-producer** closure (capturing `Arc<dyn EditorBridge>` + the `EditorRenderState`) into `BaseviewGlSkiaFrameHandler` so `on_frame`/`on_event` drive `EditorRenderState::render` per cadence tick — (a) user input, (b) host param change, (c) per presented frame for meters (vsync, never per audio block); move scene-build to `Editor::open` (where the bridge exists; the deferred lifecycle change, so `from_entry_script` retains source + template + routing); per-cycle **failure policy** — a throwing re-invocation keeps the last-good (or error) scene, never propagates a panic into the host UI thread (`render` returns `Result`, the producer catches); verify the closure satisfies baseview's `WindowHandler` `Send`/`'static` bound (boa now runs on the UI thread each frame); flag (do not necessarily adopt) the parsed-program cache that drops the per-frame re-parse.
- [ ] Dependencies: `0009.4a`. **Verification note:** unlike 4a, 4b needs a live window — its only proof is the gated `native_truce_editor_smoke`-class path under `HAWK2UI_NATIVE_BASEVIEW_SMOKE=1` on **Xvfb** (off by default, not in `check-fast`). Sits next to the cross-cutting presentation landmines (macOS/Windows parent handles, FBO resize/DPI).
- [ ] Verify: `HAWK2UI_NATIVE_BASEVIEW_SMOKE=1 cargo test -p hawk2ui-plugin-truce --test native_truce_editor_smoke` under Xvfb (presents from-script frames reflecting a param change) · `cargo test -p hawk2ui-smoke --test smoke_apps`.
