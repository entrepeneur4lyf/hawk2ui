...
- [x] Verify: **`HAWK2UI_NATIVE_BASEVIEW_SMOKE=1 xvfb-run -a cargo test -p hawk2ui-plugin-truce --test native_truce_editor_smoke`** — under Xvfb the producer-driven loop **runs clean** in a real X11 window: it presents frames with no error (a failed cycle would set `has_error`, so green = a clean live cycle), and a **reopen** (close → open the same instance) presents new frames, guarding the render-state-clone fix. Plus no-window unit tests: only a script-built editor retains a live render state, and `build_scene_producer` clones (not consumes) it. `check-fast` + clippy pedantic green. **Caveat (runs ≠ reflects):** the smoke uses an empty snapshot/routing, so the real-bridge→`refresh_snapshot_from_bridge`→window seam is proven to *run*, not to *reflect* a value change in pixels; the reflect path is covered in 4a's fake-bridge unit tests, so the two meet only in unit-land.
...
**Latent, deferred consciously (write down, don't forget — surface once input lands):**
1. The loop replays edits **unconditionally every frame**. Harmless today (the entry only emits edits on input, which isn't wired), but once input lands an author who calls `setParam` outside an input gate will storm the bridge at vsync. The input wiring must gate edit emission to actual input events.
2. `build_scene_producer`'s degrade records a failed cycle into `last_error`, which is **sticky** — `has_error` never clears, so one transient mid-session script throw flags the editor errored forever even though it correctly keeps presenting. Entangled with `last_error`'s dual use (it also holds fatal GL-init errors that *should* persist); revisit when separating transient-render from fatal state.
