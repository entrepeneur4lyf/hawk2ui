    ...
    fn closed(window_info: WindowInfo) -> Self {
        ...
        Self {
            state: Rc::new(WindowState {
                window_inner,
                window_handler: RefCell::new(Box::new(ClosedWindowHandler)),
...
struct ClosedWindowHandler;

impl WindowHandler for ClosedWindowHandler {
    fn on_frame(&mut self, _window: &mut crate::Window) {}

    fn on_event(&mut self, _window: &mut crate::Window, _event: Event) -> EventStatus {
        EventStatus::Ignored
    }
}
    ...
    pub(super) fn close(&self) {
        if self.open.get() {
            ...
            autoreleasepool(|_| unsafe {
                ...
                if let Some(state_ivar) = ns_view.class().instance_variable(BASEVIEW_STATE_IVAR) {
                    let state_ptr: *const c_void = *state_ivar.load::<*const c_void>(&ns_view);
...
impl<'a> Window<'a> {
        ...
        B: Send + 'static,
    {
        autoreleasepool(|_| {
            ...
            let handle = if let RawWindowHandle::AppKit(handle) = parent.raw_window_handle() {
                ...
                return WindowHandle::closed(window_info);
            };
        ...
        B: Send + 'static,
    {
        ...
        let Some(ns_view) = window_inner.ns_view.get() else {
            return WindowHandle::closed(window_info);
        };
    ...
    pub fn has_focus(&mut self) -> bool {
        let Some(view) = self.inner.ns_view.get() else {
            return false;
        };
...
impl WindowState {
    ...
    pub(super) unsafe fn from_view(view: &NSView) -> Rc<WindowState> {
        let Some(state_ivar) = view.class().instance_variable(BASEVIEW_STATE_IVAR) else {
            std::process::abort();
        };
        let state_ptr = state_ivar.load::<*const c_void>(view).cast::<WindowState>();
