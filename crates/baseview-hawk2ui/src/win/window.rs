...
impl WindowHandle {
    fn closed() -> Self {
        Self { hwnd: None, is_open: Rc::new(Cell::new(false)) }
    }
...
impl WindowImpl for BaseviewWindow {
    fn after_create(&self, window: HWnd) -> Result<()> {
        ...
        if let Some(gl_config) = self.gl_config.clone() {
            ...
            let Ok(gl_context) = (unsafe { GlContext::create(&handle, gl_config) }) else {
...
impl Window<'_> {
        ...
        B: Send + 'static,
    {
        ...
        let Ok(hwnd) = create_window(
            ...
            is_open.set(false);
            return WindowHandle { hwnd: None, is_open };
        };
...
pub fn copy_to_clipboard(_data: &str) {
    // Clipboard integration is intentionally best-effort for this legacy fork path.
}
