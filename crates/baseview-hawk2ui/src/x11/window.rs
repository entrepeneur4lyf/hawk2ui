...
impl<'a> Window<'a> {
        ...
        B: Send + 'static,
    {
        ...
        let join_handle = thread::spawn(move || {
            if Self::window_thread(Some(parent_id), options, build, tx.clone(), Some(parent_handle))
                .is_err()
            {
                let _ = tx.send(Err(()));
            }
        ...
        B: Send + 'static,
    {
        ...
        let thread = thread::spawn(move || {
            if Self::window_thread(None, options, build, tx.clone(), None).is_err() {
                let _ = tx.send(Err(()));
            }
        });

        let Ok(Ok(_)) = rx.recv() else {
            let _ = thread.join();
            return;
        };

        let _ = thread.join().map_err(|err| {
        ...
        B: Send + 'static,
    {
        ...
        #[cfg(feature = "opengl")]
        let gl_context = gl_context.flatten();
        ...
        let mut window =
            crate::Window::new(crate::linux::Window::X11(Window { inner: &mut inner }));
    ...
    pub fn set_mouse_cursor(&self, mouse_cursor: MouseCursor) {
        ...
        let Ok(xid) = self.inner.xcb_connection.get_cursor(mouse_cursor) else {
            return;
        };
...
pub fn copy_to_clipboard(_data: &str) {
    // Clipboard integration is host-provided for this Linux embedding path.
}
