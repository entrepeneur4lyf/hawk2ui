...
impl EventLoop {
    ...
    pub fn run(&mut self) -> Result<(), Box<dyn Error>> {
        ...
        while self.event_loop_running {
            ...
            if Instant::now() >= next_frame {
                self.handler.on_frame(&mut crate::Window::new(crate::linux::Window::X11(Window {
                    inner: &self.window,
                })));
            ...
            if let Some(revents) = fds[0].revents() {
                if revents.contains(PollFlags::POLLERR) {
                    return Err("xcb connection poll error".into());
                }
                ...
                4..=7 => {
                    self.handle_event(Event::Mouse(MouseEvent::WheelScrolled {
                        delta: match event.detail {
                            ...
                            _ => ScrollDelta::Lines { x: 0.0, y: 0.0 },
                        },
                        modifiers: key_mods(event.state),
                    }));
                }
    ...
    fn handle_event(&mut self, event: Event) {
        self.handler.on_event(
            &mut crate::Window::new(crate::linux::Window::X11(Window { inner: &self.window })),
            event,
        );
    }
