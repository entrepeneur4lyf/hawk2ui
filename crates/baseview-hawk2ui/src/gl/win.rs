...
impl GlContext {
    pub unsafe fn create(parent: &RawWindowHandle, config: GlConfig) -> Result<GlContext, GlError> {
        ...
        let Some(wgl_choose_pixel_format_arb) = wglChoosePixelFormatARB else {
            ReleaseDC(hwnd, hdc);
            return Err(GlError::CreationFailed(()));
        };

        let Some(wgl_create_context_attribs_arb) = wglCreateContextAttribsARB else {
            ReleaseDC(hwnd, hdc);
            return Err(GlError::CreationFailed(()));
        };
        ...
        if let Some(wgl_swap_interval_ext) = wglSwapIntervalEXT {
