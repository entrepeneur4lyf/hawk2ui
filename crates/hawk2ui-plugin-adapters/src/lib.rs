...
static ACTIVE_EDITOR_INSTANCE: AtomicUsize = AtomicUsize::new(0);
...
struct Hawk2uiPluginInstance {
    plugin: clap_plugin,
    editor_created: AtomicBool,
    editor_attached: AtomicBool,
    editor_visible: AtomicBool,
    editor_width: AtomicU32,
    editor_height: AtomicU32,
    editor_presented_frame_count: AtomicU64,
    parameter_values: [AtomicU64; __PARAMETER_COUNT__],
}

impl Hawk2uiPluginInstance {
    fn new() -> Box<Self> {
        let mut instance = Box::new(Self {
            ...
            editor_created: AtomicBool::new(false),
            editor_attached: AtomicBool::new(false),
            editor_visible: AtomicBool::new(false),
            editor_width: AtomicU32::new(__EDITOR_WIDTH__),
            editor_height: AtomicU32::new(__EDITOR_HEIGHT__),
            editor_presented_frame_count: AtomicU64::new(0),
            parameter_values: __PARAMETER_VALUES__,
        });
        let instance_ptr = (&mut *instance as *mut Self).cast();
        instance.plugin.plugin_data = instance_ptr;
        instance
    }
}
...
unsafe extern "C" fn create_plugin(
    ...
    if requested.to_bytes() == expected {
        let instance = Hawk2uiPluginInstance::new();
        let instance_ptr = Box::into_raw(instance);
        ACTIVE_EDITOR_INSTANCE.store(instance_ptr as usize, Ordering::Release);
        unsafe { &(*instance_ptr).plugin }
    } else {
        std::ptr::null()
    }
}

unsafe fn plugin_instance<'a>(plugin: *const clap_plugin) -> Option<&'a Hawk2uiPluginInstance> {
    if plugin.is_null() {
        return None;
    }
    let plugin = unsafe { &*plugin };
    let data = plugin.plugin_data.cast::<Hawk2uiPluginInstance>();
    if data.is_null() {
        return None;
    }
    Some(unsafe { &*data })
...
fn active_editor_instance<'a>() -> Option<&'a Hawk2uiPluginInstance> {
    let instance = ACTIVE_EDITOR_INSTANCE.load(Ordering::Acquire);
    if instance == 0 {
        return None;
    }
    Some(unsafe { &*(instance as *const Hawk2uiPluginInstance) })
}

fn set_active_editor_instance(instance: &Hawk2uiPluginInstance) {
    ACTIVE_EDITOR_INSTANCE.store(instance as *const Hawk2uiPluginInstance as usize, Ordering::Release);
}

unsafe extern "C" fn plugin_init(plugin: *const clap_plugin) -> bool {
    unsafe { plugin_instance(plugin) }.is_some()
}
...
unsafe extern "C" fn plugin_destroy(plugin: *const clap_plugin) {
    ...
    let data = unsafe { (*plugin).plugin_data.cast::<Hawk2uiPluginInstance>() };
    if data.is_null() {
        return;
    }
    let active = ACTIVE_EDITOR_INSTANCE.load(Ordering::Acquire);
    if active == data as usize {
        ACTIVE_EDITOR_INSTANCE.store(0, Ordering::Release);
    }
...
unsafe extern "C" fn plugin_activate(
    ...
    unsafe { plugin_instance(_plugin) }.is_some()
...
unsafe extern "C" fn plugin_process(
    ...
    if unsafe { plugin_instance(plugin) }.is_none() || process.is_null() {
        return CLAP_PROCESS_ERROR;
    }
    if !realtime_policy_self_check() {
        return CLAP_PROCESS_ERROR;
    }
...
fn realtime_policy_self_check() -> bool {
    realtime_operation_allowed(Hawk2uiRealtimeOperation::PreallocatedWrite)
        && !realtime_operation_allowed(Hawk2uiRealtimeOperation::Allocation)
        && !realtime_operation_allowed(Hawk2uiRealtimeOperation::BlockingWait)
}

fn realtime_operation_allowed(operation: Hawk2uiRealtimeOperation) -> bool {
...
unsafe extern "C" fn params_get_info(
    ...
    unsafe extern "C" fn params_get_value(
        ...
        unsafe {
            *out_value = f64::from_bits(instance.parameter_values[index].load(Ordering::Acquire));
        }
        true
    }
...
unsafe extern "C" fn params_text_to_value(
    ...
    unsafe extern "C" fn params_flush(
        ...
        for index in 0..event_count {
            ...
            let _ = store_parameter_value(instance, event.param_id, event.value);
        }
    }
    ...
    fn store_parameter_value(instance: &Hawk2uiPluginInstance, param_id: u32, value: f64) -> bool {
        if !value.is_finite() {
            return false;
        }
        let Some(index) = find_parameter_index(param_id) else {
        ...
        instance.parameter_values[index].store(
...
fn write_c_buffer(out_buffer: *mut c_char, out_buffer_capacity: u32, source: &[u8]) {
    ...
    unsafe extern "C" fn state_save(plugin: *const clap_plugin, stream: *const clap_ostream) -> bool {
        ...
        let state = state_payload(instance);
...
unsafe extern "C" fn state_load(plugin: *const clap_plugin, stream: *const clap_istream) -> bool {
    ...
    let Some(read) = stream.read else {
        ...
        restore_state_payload(instance, &payload)
    }

    fn state_payload(instance: &Hawk2uiPluginInstance) -> Vec<u8> {
        let mut state = String::from(std::str::from_utf8(STATE_BYTES).unwrap_or("hawk2ui-state-v1\n"));
        for (index, parameter) in PARAMETERS.iter().enumerate() {
            let bits = instance.parameter_values[index].load(Ordering::Acquire);
    ...
    fn restore_state_payload(instance: &Hawk2uiPluginInstance, payload: &[u8]) -> bool {
        ...
        for line in state.lines().skip(1) {
            ...
            if !store_parameter_value(instance, param_id, value) {
                return false;
            }
        }
        true
...
unsafe extern "C" fn gui_create(
    ...
    set_active_editor_instance(instance);
    instance.editor_created.store(true, Ordering::Release);
    instance.editor_attached.store(false, Ordering::Release);
    instance.editor_visible.store(false, Ordering::Release);
    true
}
...
unsafe extern "C" fn gui_destroy(plugin: *const clap_plugin) {
    ...
    instance.editor_created.store(false, Ordering::Release);
    instance.editor_attached.store(false, Ordering::Release);
    instance.editor_visible.store(false, Ordering::Release);
}
...
unsafe extern "C" fn gui_get_size(
    ...
    unsafe {
        *width = instance.editor_width.load(Ordering::Acquire);
        *height = instance.editor_height.load(Ordering::Acquire);
    }
    true
}
...
unsafe extern "C" fn gui_set_size(plugin: *const clap_plugin, width: u32, height: u32) -> bool {
    ...
    instance.editor_width.store(width, Ordering::Release);
    instance.editor_height.store(height, Ordering::Release);
    true
}
...
unsafe extern "C" fn gui_set_parent(
    ...
    window: *const clap_window,
      ...
      if !instance.editor_created.load(Ordering::Acquire)
      ...
      set_active_editor_instance(instance);
      instance.editor_attached.store(true, Ordering::Release);
    true
}
...
unsafe extern "C" fn gui_show(plugin: *const clap_plugin) -> bool {
    ...
    let can_show = instance.editor_created.load(Ordering::Acquire)
        && instance.editor_attached.load(Ordering::Acquire);
    if can_show {
        set_active_editor_instance(instance);
        instance.editor_visible.store(true, Ordering::Release);
    }
    can_show
}
...
unsafe extern "C" fn gui_hide(plugin: *const clap_plugin) -> bool {
    ...
    let was_created = instance.editor_created.load(Ordering::Acquire);
    if was_created {
        instance.editor_visible.store(false, Ordering::Release);
    }
    was_created
...
#[unsafe(no_mangle)]
  ...
  #[unsafe(no_mangle)]
  pub unsafe extern "C" fn hawk2ui_editor_dispatch_for_plugin(
      plugin: *const clap_plugin,
      ...
      dispatch_editor_command(instance, command_ptr, command_len, response_ptr, response_capacity, response_len)
  }
  ...
  pub unsafe extern "C" fn hawk2ui_editor_dispatch(
      ...
      let Some(instance) = active_editor_instance() else {
          return false;
      };
      dispatch_editor_command(instance, command_ptr, command_len, response_ptr, response_capacity, response_len)
  }
  ...
  fn dispatch_editor_command(
      instance: &Hawk2uiPluginInstance,
      command_ptr: *const u8,
      command_len: usize,
      response_ptr: *mut u8,
      response_capacity: usize,
      ...
      let response = match command_name {
          "create" => dispatch_create(instance, command),
          "set_parent" => dispatch_set_parent(instance, command),
          "show" => dispatch_show(instance),
          "hide" => dispatch_hide(instance),
          "destroy" => dispatch_destroy(instance),
          "apply_parameter" => dispatch_apply_parameter(instance, command),
          "save_state" => dispatch_save_state(instance),
          "load_state" => dispatch_load_state(instance, command),
  ...
  fn dispatch_create(instance: &Hawk2uiPluginInstance, command: &str) -> String {
      ...
      instance.editor_attached.store(false, Ordering::Release);
      instance.editor_visible.store(false, Ordering::Release);
      instance.editor_presented_frame_count.store(0, Ordering::Release);
      "response=created\n".to_owned()
  }

  fn dispatch_set_parent(instance: &Hawk2uiPluginInstance, command: &str) -> String {
      if !instance.editor_created.load(Ordering::Acquire) {
  ...
  fn dispatch_show(instance: &Hawk2uiPluginInstance) -> String {
      if !instance.editor_created.load(Ordering::Acquire) {
          return "error=editor-not-created\n".to_owned();
      }
      if !instance.editor_attached.load(Ordering::Acquire) {
          return "error=editor-not-attached\n".to_owned();
      }
      set_active_editor_instance(instance);
      instance.editor_visible.store(true, Ordering::Release);
      let presented_frame_count = instance.editor_presented_frame_count.fetch_add(1, Ordering::AcqRel) + 1;
      format!(
          "response=frame_presented\nwidth={}\nheight={}\npresented_frame_count={presented_frame_count}\n",
          instance.editor_width.load(Ordering::Acquire),
          instance.editor_height.load(Ordering::Acquire)
      )
  }

  fn dispatch_hide(instance: &Hawk2uiPluginInstance) -> String {
      if !instance.editor_created.load(Ordering::Acquire) {
          return "error=editor-not-created\n".to_owned();
      }
      instance.editor_visible.store(false, Ordering::Release);
  ...
  fn dispatch_destroy(instance: &Hawk2uiPluginInstance) -> String {
      instance.editor_created.store(false, Ordering::Release);
      instance.editor_attached.store(false, Ordering::Release);
      instance.editor_visible.store(false, Ordering::Release);
      "response=destroyed\n".to_owned()
  ...
  fn dispatch_apply_parameter(instance: &Hawk2uiPluginInstance, command: &str) -> String {
      ...
      if !store_parameter_value(instance, parameter_id, value) {
          return "error=invalid-parameter-value\n".to_owned();
      }
      let Some(index) = find_parameter_index(parameter_id) else {
          return "error=invalid-parameter-id\n".to_owned();
      };
      let stored_value = f64::from_bits(instance.parameter_values[index].load(Ordering::Acquire));
      format!("response=parameter_applied\nparameter_id={parameter_id}\nvalue={stored_value}\n")
  }

  fn dispatch_save_state(instance: &Hawk2uiPluginInstance) -> String {
  ...
  fn dispatch_load_state(instance: &Hawk2uiPluginInstance, command: &str) -> String {
      ...
      for parameter in PARAMETERS {
          ...
          if let Some(bits) = dispatch_field(command, &key).and_then(|value| value.parse::<u64>().ok()) {
              if store_parameter_value(instance, parameter.id, f64::from_bits(bits)) {
                  loaded_count += 1;
              }
          }
      }
  ...
  #[unsafe(no_mangle)]
  pub unsafe extern "C" fn hawk2ui_editor_state_for_plugin(plugin: *const clap_plugin) -> Hawk2uiEditorState {
      unsafe { plugin_instance(plugin) }
          .map(editor_state_for_instance)
          .unwrap_or_else(empty_editor_state)
  }

  #[unsafe(no_mangle)]
  pub unsafe extern "C" fn hawk2ui_editor_state() -> Hawk2uiEditorState {
      active_editor_instance()
          .map(editor_state_for_instance)
          .unwrap_or_else(empty_editor_state)
  }

  fn editor_state_for_instance(instance: &Hawk2uiPluginInstance) -> Hawk2uiEditorState {
      Hawk2uiEditorState {
          created: instance.editor_created.load(Ordering::Acquire),
          attached: instance.editor_attached.load(Ordering::Acquire),
          visible: instance.editor_visible.load(Ordering::Acquire),
          width: instance.editor_width.load(Ordering::Acquire),
          height: instance.editor_height.load(Ordering::Acquire),
      }
  }

  fn empty_editor_state() -> Hawk2uiEditorState {
      Hawk2uiEditorState {
          created: false,
          attached: false,
          visible: false,
          width: __EDITOR_WIDTH__,
          height: __EDITOR_HEIGHT__,
      }
  }
"#;
...
impl Class for Hawk2uiVst3Factory {
    type Interfaces = (IPluginFactory,);
}

impl IPluginFactoryTrait for Hawk2uiVst3Factory {
    unsafe fn getFactoryInfo(&self, info: *mut PFactoryInfo) -> tresult {
        ...
        copy_cstring("https://hawk2ui.local", &mut info.url);
    ...
    unsafe fn getClassInfo(&self, index: i32, info: *mut PClassInfo) -> tresult {
        ...
        match index {
            0 => {
                info.cid = class_id_tuid(PROCESSOR_CLASS_ID_HEX);
                info.cardinality = PClassInfo_::ClassCardinality_::kManyInstances as int32;
                copy_cstring(Vst3ClassCategory::AudioModule.as_vst3_category(), &mut info.category);
                copy_cstring(PLUGIN_NAME, &mut info.name);
                kResultOk
            }
            1 => {
                info.cid = class_id_tuid(CONTROLLER_CLASS_ID_HEX);
                info.cardinality = PClassInfo_::ClassCardinality_::kManyInstances as int32;
                copy_cstring(
                    Vst3ClassCategory::ComponentController.as_vst3_category(),
                    &mut info.category,
                );
                copy_cstring(PLUGIN_NAME, &mut info.name);
                kResultOk
...
fn copy_cstring(source: &str, target: &mut [c_char]) {
    if target.is_empty() {
        return;
    }
    let write_len = source.len().min(target.len().saturating_sub(1));
    for (index, byte) in source.as_bytes().iter().take(write_len).enumerate() {
        target[index] = *byte as c_char;
    }
    target[write_len] = 0;
}
...
fn class_id_tuid(hex: &str) -> TUID {
    ...
    for (index, byte) in tuid.iter_mut().enumerate() {
        let offset = index * 2;
        if offset + 2 <= hex.len() {
            *byte = u8::from_str_radix(&hex[offset..offset + 2], 16).unwrap_or(0) as c_char;
        }
    }
    tuid
}
...
extern "system" fn GetPluginFactory() -> *mut IPluginFactory {
    ComWrapper::new(Hawk2uiVst3Factory)
...
impl PackagePlan {
    ...
    /// Materializes package output directories with deterministic metadata and generated scaffolds.
    ///
    /// This writes source scaffolds and bundle descriptors. It does not compile final plugin or
    /// application binaries; callers must build the generated source packages as a separate release
    /// step.
    ///
    /// # Errors
    ///
...
fn parameter_default_value(parameter: &ParameterRecord) -> f64 {
    match parameter.default_value {
        ...
        ParameterValue::Choice(index) => f64::from(index),
...
fn clap_parameter_flags(parameter: &ParameterRecord) -> u32 {
    let mut flags = 0;
    if parameter.steps.is_some() || matches!(parameter.default_value, ParameterValue::Choice(_)) {
